...
module SolidApm
  class ImpactfulTransactionsResource < ApplicationMcpResource
    ...
    def calculate_impactful_transactions
      ...
      transaction_groups = SolidApm::Transaction
        .includes(:spans)
        .where('timestamp >= ?', cutoff_time)
        .group_by { |t| [t.name, t.type] }
      
      impact_data = transaction_groups.map do |group_key, transactions|
        name, type = group_key
